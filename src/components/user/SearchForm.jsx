...
import { FaExchangeAlt } from 'react-icons/fa'; // Import FaExchangeAlt
...
export default function SearchForm() {
    const getDefaultDate = () => {
        let date = new Date();
        return date.toISOString().split('T')[0]; // Returns the date in 'YYYY-MM-DD' format for input type="date"
    };
    const { register, handleSubmit, formState: { errors } } = useForm();
    const [selectedDate, setSelectedDate] = useState(getDefaultDate()); // Set default date
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const { pickupLocation, dropoffLocation, date } = data; // Destructure data
            const response = await axiosInstance.get(`/bus/search?pickupLocation=${pickupLocation}&dropoffLocation=${dropoffLocation}&date=${date}`);
            setTimeout(() => {
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="flex flex-col md:flex-row justify-center shadow-xl items-center mt-10 sm:mt-16 w-full">
                        <div className="bg-white rounded-3xl flex flex-col md:flex-row items-center shadow-lg p-4 md:p-0 w-full md:w-auto">
                            ...
                            {/* Date Section */}
                            <div className="flex items-center px-4 py-2">
                                <CalendarDays onClick={toggleCalendar} className="text-gray-500 mr-2" id='Date' />
                                <div>
                                    <div className="text-gray-500 text-sm" id='Date'>Date</div>
                                    <input
                                        type="date"
                                        id="Date"
                                        value={selectedDate} // Set value to selectedDate
                                        ...
                                        min={getDefaultDate()} // Set the minimum date to today
                                    />
                            ...
                            <button
                                type="submit"
                                disabled={isLoading}
                                className={`bg-red-600 h-24 hover:bg-red-700 transition-all duration-300 text-white font-semibold rounded-l-full rounded-r-3xl px-8 py-4 mt-4 md:mt-0 md:ml-4 md:py-2 md:px-6 w-full md:w-auto ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                                {isLoading ? (
